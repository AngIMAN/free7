...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/AngIMAN/registrasi/main/regis > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/AngIMAN/registrasi/main/regis | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/AngIMAN/registrasi/main/regis | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/AngIMAN/registrasi/main/regis | grep $MYIP | awk '{print $3}')
fi
